...
Then we consider the bounds for $f(x)$, $f(y)$ and $f(x) - f(y)$. For each function, we compute the minimum $L_1$ bounds using the convex (concave) envelope of the function in the selected parallelogram area. The case for the Parallelogram 1 ($l_x \le x \le u_x, l_y \le y \le u_y$) is the simplest, we can directly compute the convex (concave) envelope of $f$ in the range of $x$ and $y$. The other two cases are symmetric, so we only consider the case for the Parallelogram 2 ($l_x \le x \le u_x, l_d \le x - y \le u_d$) in our documentation. 
