...
Note that with token merging, more complicated bounds may be needed in this notion, e.g., suppose $x$ is a token merged from $y$, $z$ in another network, we need bounds like: $l_x \le x \le u_x, l_y \le y \le u_y, l_z \le z \le u_z, l_d \le x - (y + z) / 2 \le u_d$.
